...
struct Types : public ASTNode {
  ...
  virtual std::unique_ptr<Types> clone() const = 0;
};
...
struct Parameter : public ASTNode {
  std::string name;
  TypePtr type;
  Parameter(std::string n, TypePtr t)
...
struct RangeExpr : public Expression {
  ...
  RangeExpr(ExprPtr l, ExprPtr r, bool inc)
      : left(std::move(l)), right(std::move(r)), inclusive(inc) {}
  void print(int level) override;
};
...
struct BinaryExpression : public Expression {
  ...
  ExprPtr left;
  ExprPtr right;
  BinaryExpression(BinaryOp o, ExprPtr l, ExprPtr r)
...
struct IndexExpression : public Expression {
  ExprPtr object;
  ExprPtr index;
  IndexExpression(ExprPtr o, ExprPtr i)
...
struct FunctionCallExpr : public Expression {
  ...
  FunctionCallExpr(std::string c, std::vector<ExprPtr> args)
...
struct FunctionDecl : public ASTNode {
  ...
  std::unique_ptr<StatementBlock> body;
  FunctionDecl(std::string n, std::vector<ParameterPtr> p,
               std::vector<TypePtr> r, std::unique_ptr<StatementBlock> b)
      : name(std::move(n)), params(std::move(p)), return_types(std::move(r)),
        body(std::move(b)) {}
  void print(int level) override;
};
//TODO: more robust struct
